...
export const changePassword = async(req,res) => {
    try {
        ...
        if (isMatch) { //if current passwords match allow user to change password
        const salt = await bcrypt.genSalt(10)
        const hashedPassword = await bcrypt.hash(newPassword, salt) //salt and hash the new password
